...
globalThis.ceylonSwiftAuth = Object.freeze({ mode: config.authMode, login, requestOtp, verifyOtp, startGoogleLogin, request: (path, options) => service.request(path, options), logout, logoutAll: () => logout(true), selectWorkspace: selectWorkspaceAndRoute, openWorkspaceSelector: () => showWorkspaceSelector(service.state.snapshot.workspaces, false), getState: () => service.state.snapshot });
