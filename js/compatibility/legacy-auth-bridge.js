...
import { AuthenticatedShell } from '../shell/authenticated-shell.js';
import { AuthViewController } from '../auth/auth-view-controller.js';
...
function sync(snapshot) { globalThis.applyBackendAuthCompatibility?.({ status: snapshot.status, role: compatibleRole(snapshot), roles: backendRoles(snapshot), user: snapshot.user, workspace: snapshot.activeWorkspace, capabilities: snapshot.capabilities, capabilityStatus: snapshot.capabilityStatus, workspaceSelectionPending: snapshot.status === 'authenticated' && snapshot.workspaces.length > 1 && !snapshot.activeWorkspace, mode: config.authMode }); document.documentElement.dataset.authStatus = snapshot.status; shell.render(snapshot); if (snapshot.lastError?.message) renderMessage(snapshot.lastError.message, snapshot.status === 'unavailable' ? 'warning' : 'error'); else if (snapshot.capabilityError?.message) renderMessage('Your workspace permissions could not be loaded. Try switching workspace or sign in again.', 'warning'); else renderMessage(); }
async function attempt(operation) { setBusy(true); renderMessage(); try { return await operation(); } catch (error) { const safe = service.safeError(error); renderMessage(`${safe.message}${safe.requestId ? ` Reference: ${safe.requestId}` : ''}`); throw error; } finally { setBusy(false); } }
async function login({ identifier, password, selectedUiRole }) { const snapshot = await attempt(() => service.login({ identifier, password, selectedUiRole })); if (snapshot.status === 'authenticated') { authViewController?.close(); closeModal('authPortalModal'); renderMessage(); } return snapshot; }
...
function showWorkspaceSelector(workspaces, required = false) { workspaceSelector.open(workspaces, { activeWorkspace: service.state.snapshot.activeWorkspace, required, onSelect: id => attempt(() => service.selectWorkspace(id)) }); }
...
globalThis.ceylonSwiftAuth = Object.freeze({ mode: config.authMode, login, requestOtp, verifyOtp, startGoogleLogin, logout, logoutAll: () => logout(true), selectWorkspace: id => attempt(() => service.selectWorkspace(id)), openWorkspaceSelector: () => showWorkspaceSelector(service.state.snapshot.workspaces, false), getState: () => service.state.snapshot });
